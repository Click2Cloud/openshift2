...
$IISHWC_PID_FILE = Join-Path $env:OPENSHIFT_WINDIY_DIR 'run\iishwc.pid'
...
function start-cartridge
{
    ...
    else
    {
        ...
        $logDir = (Join-Path $env:OPENSHIFT_WINDIY_DIR 'log')
        New-Item -path $logDir -type directory -Force | Out-Null

        $job = Start-Process -PassThru -WindowStyle Hidden 'cmd' "/c $env:OPENSHIFT_WINDIY_DIR\bin\iishwc\start.bat  1>> ${logDir}\stdout.log 2>> ${logDir}\stderr.log"
        $job.Id > $IISHWC_PID_FILE

		 Write-Host "Cartridge Started From Control Script"
    }
...
function stop-cartridge
{
    if (process_running "cmd" $IISHWC_PID_FILE)
    {
        ...
        #kill /F /T /PID $jobid
		stop-process -id $jobid -passthru -Force
...
switch ($command)
  {
	...
	"build" { . $env:OPENSHIFT_WINDIY_DIR\bin\build.ps1 }
  }