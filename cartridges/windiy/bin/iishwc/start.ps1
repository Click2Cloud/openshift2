...
$script:scriptPath = Join-Path (Join-Path $env:OPENSHIFT_WINDIY_DIR bin) iishwc
$script:appPoolName = [Guid]::NewGuid().ToString()
$script:appPort = $env:OPENSHIFT_WINDIY_PORT
...
#    $healthMonitoring.SetAttribute("configSource", "RGenAspNetEventProvider.config")
#    $null = $systemWeb.AppendChild($healthMonitoring)
#}

$webConfig.Save($webConfigPath)

# New-Item -Path (Join-Path $env:OPENSHIFT_WINDIY_DIR '\tmp\IIS Temporary Compressed Files') -Force -ErrorAction SilentlyContinue -Type Directory | Out-Null
